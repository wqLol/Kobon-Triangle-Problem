...
root = tk.Tk()
root.title("Python Kobon Triangle Visualization")
...
show_points = False  # Start with original points hidden
...
triangle_colors = [random_color() for _ in triangles]
...
def redraw():
    ...
    for p1, p2 in lines_to_draw:
        ...
        candidates = []
        if B != 0:
            y_left = (-C - A * Decimal(left)) / B
            if top <= float(y_left) <= bottom:
                candidates.append((left, float(y_left)))
            y_right = (-C - A * Decimal(right)) / B
            if top <= float(y_right) <= bottom:
                candidates.append((right, float(y_right)))
        if A != 0:
            x_top = (-C - B * Decimal(top)) / A
            if left <= float(x_top) <= right:
                candidates.append((float(x_top), top))
            x_bottom = (-C - B * Decimal(bottom)) / A
            if left <= float(x_bottom) <= right:
    ...
    for tri, color in zip(triangles, triangle_colors):
